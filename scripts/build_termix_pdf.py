...
def decorate(canvas, doc):
    ...
    canvas.setFont(FONT, 7); canvas.setFillColor(MUTED); canvas.drawString(18*mm, 9*mm, "MANDATE · Final Agent Advantage Report · 27 Aug 2026")
    canvas.drawRightString(A4[0]-18*mm, 9*mm, f"Page {doc.page}"); canvas.restoreState()


def make_pdf():
    data = json.loads(DATA.read_text(encoding="utf-8")); summary, rows = data["summary"], data["rows"]; s = styles()
    ...
    t=LongTable(table_rows,repeatRows=1,colWidths=[52*mm,20*mm,20*mm,18*mm,24*mm,40*mm]); t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),INK),("GRID",(0,0),(-1,-1),.5,LINE),("VALIGN",(0,0),(-1,-1),"TOP"),("ROWBACKGROUNDS",(0,1),(-1,-1),[WHITE,PAPER]),("PADDING",(0,0),(-1,-1),3*mm)])); story += [t, Spacer(1,5*mm), Paragraph("Interpretation",s["h2"]), Paragraph("The largest cold-start penalty is preserved: A-01 took 2.9864 seconds rather than the sub-second A-02 and A-03 runs. Even with that untrimmed production latency, all three tasks show large time savings and higher rubric scores. Results demonstrate bounded analytical workflow execution, not autonomous custody or real-capital performance.",s["body"]), PageBreak(), Paragraph("Onchain marketplace evidence",s["h1"])]
    for r in rows:
